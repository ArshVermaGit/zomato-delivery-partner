...
config.resolver.resolveRequest = (context, moduleName, platform) => {
    const isExpoGoMock = process.env.EXPO_GO_MOCK === 'true';

    // --- Mock Redirections ---
    if (isExpoGoMock) {
        if (moduleName === '@notifee/react-native') {
            return {
                filePath: path.resolve(projectRoot, 'mocks/notifee.js'),
                type: 'sourceFile',
            };
        }
        if (moduleName === 'react-native-reanimated') {
            return {
                filePath: path.resolve(projectRoot, 'mocks/reanimated.js'),
                type: 'sourceFile',
            };
        }
    }
